...
public class PostUsersAPITest extends BaseTest {

	String id;

	@Test(priority = 1)
	public void validUsersAPITest() {
		...
		Response response = RestAssured.given().when().headers(header.defaultHeaders())
				.body(builder.postUsresRequestBody(JavaUtilis.randomNumebr(), JavaUtilis.randomStrig(),
						JavaUtilis.randomStrig(), JavaUtilis.randomStrig()))
				.when().post(APIPath.apiPath.CREATE_USERS);
